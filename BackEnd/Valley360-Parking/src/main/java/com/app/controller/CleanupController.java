...
public class CleanupController {
    ...
    public ResponseEntity<List<InactiveResourceDTO>> getInactiveAreas(
        ...
        return ResponseEntity.ok(cleanupService.getInactiveParkingAreas(daysInactive));
    }
    ...
    public ResponseEntity<List<InactiveResourceDTO>> getInactiveSlots(
        ...
        return ResponseEntity.ok(cleanupService.getInactiveParkingSlots(daysInactive));
    }

    @GetMapping("/summary")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<CleanupReportDTO> getCleanupSummary() {
        return ResponseEntity.ok(cleanupService.getCleanupSummary());
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> deleteParkingArea(@PathVariable Long areaId) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> archiveParkingArea(@PathVariable Long areaId) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> restoreParkingArea(@PathVariable Long areaId) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> markAreaAsActive(@PathVariable Long areaId) {
        log.info("Marking parking area as active: {}", areaId);
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> archiveParkingSlot(@PathVariable Long slotId) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> restoreParkingSlot(@PathVariable Long slotId) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> markSlotAsActive(@PathVariable Long slotId) {
        log.info("Marking parking slot as active: {}", slotId);
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> bulkDeleteAreas(@RequestBody Map<String, List<Long>> request) {
        List<Long> areaIds = request.get("areaIds");
        log.info("Bulk deleting {} parking areas", areaIds.size());
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> bulkArchiveAreas(@RequestBody Map<String, List<Long>> request) {
        List<Long> areaIds = request.get("areaIds");
        log.info("Bulk archiving {} parking areas", areaIds.size());
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> bulkDeleteSlots(@RequestBody Map<String, List<Long>> request) {
        List<Long> slotIds = request.get("slotIds");
        log.info("Bulk deleting {} parking slots", slotIds.size());
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<Void> bulkArchiveSlots(@RequestBody Map<String, List<Long>> request) {
        List<Long> slotIds = request.get("slotIds");
        log.info("Bulk archiving {} parking slots", slotIds.size());
    ...
    public ResponseEntity<List<InactiveResourceDTO>> getArchivedAreas() {
        return ResponseEntity.ok(cleanupService.getArchivedAreas());
    }

    @GetMapping("/archived-slots")
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<InactiveResourceDTO>> getArchivedSlots() {
        return ResponseEntity.ok(cleanupService.getArchivedSlots());
    }
    ...
    public ResponseEntity<ScheduledDeletionDTO> scheduleDeletion(
        ...
        LocalDate deletionDate = LocalDate.parse(request.get("deletionDate").toString());
        
        log.info("Scheduling deletion for {} {} on {}", resourceType, resourceId, deletionDate);
        return ResponseEntity.ok(cleanupService.scheduleDeletion(resourceType, resourceId, deletionDate));
    }

    @PostMapping("/cancel-deletion/{token}")
    public ResponseEntity<Map<String, Object>> cancelScheduledDeletion(@PathVariable String token) {
        log.info("Cancelling scheduled deletion with token: {}", token);
        boolean success = cleanupService.cancelScheduledDeletion(token);
        return ResponseEntity.ok(Map.of(
            "success", success,
            "message", success ? "Deletion cancelled successfully" : "Failed to cancel deletion"
        ));
    }
    ...
    public ResponseEntity<List<ScheduledDeletionDTO>> getPendingDeletions() {
        return ResponseEntity.ok(cleanupService.getPendingDeletions());
    }
    ...
    public ResponseEntity<CleanupReportDTO> getMonthlyReport(
        ...
        log.info("Getting monthly cleanup report for {}/{}", month, year);
        return ResponseEntity.ok(cleanupService.generateMonthlyReport(month, year));
    }
    ...
    public ResponseEntity<CleanupReportDTO> generateReport() {
        ...
        return ResponseEntity.ok(cleanupService.generateMonthlyReport(now.getMonthValue(), now.getYear()));
    }
}
